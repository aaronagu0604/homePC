...
class MemcacheSessionStore:
    def __init__(self, mcc_connection, **options):
        ...
        self.mcc.set('sid_list', [], 60 * 60 * 24 * 30)
    ...
    def set_sid_list(self, sid):
        value = self.mcc.get('sid_list')
        value.append(sid)
        self.mcc.replace('sid_list', value, 60 * 60 * 24 * 30)

    def get_sid_list(self):
        return self.mcc.get('sid_list')

    def delete_former_session(self, new_user, new_sid):
        try:
            for sid in self.mcc.get('sid_list'):
                former_user, former_sid = self.get_session(sid, 'sid')['admin']
                if new_user == former_user and new_sid != former_sid:
...
class Session:
    ...
    def save(self):
        if self._dirty:
            ...
            self._store.set_sid_list(self._sid)
            self._dirty = False
