...
class AdminUser(db.Model):
    ...
    get_msg = IntegerField(default=0)  # 是否可以接收下保单短信
...
class Store(db.Model):
    ...
    lng = FloatField(default=0.0)  # 经度坐标
    lat = FloatField(default=0.0)  # 纬度坐标
    process_car_service = IntegerField(default=0)  # 店铺是否允许提供保养服务：0否，1是
    ''' 优惠券 json 结构
    [
        {"id": 1, "name": u"单交强险", "flag": 1, "minprice":"", "maxprice":"", "ticket": [{'CarServiceBox_id': 1, 'count': 2}]},
        {"id": 2, "name": u"单商业险", "flag": 2, "minprice":"", "maxprice":"", "ticket": [{'CarServiceBox_id': 1, 'count': 2}]},
        {"id": 3, "name": u"交强险+商业险", "flag": 3, "minprice":"","maxprice":"", "ticket": [{'CarServiceBox_id': 1, 'count': 2}]}
    ]
    '''
...
# 保养分类
class CarServiceCategory(db.Model):
    id = PrimaryKeyField()
    name = CharField(max_length=20)  # 分类名
    active = IntegerField(default=1)  # 状态 0删除 1有效

    class Meta:
        db_table = 'tb_car_service_category'


# 优惠券类型
class CarServiceType(db.Model):
    id = PrimaryKeyField()
    category = ForeignKeyField(CarServiceCategory, related_name='category_types', db_column='category_id')
    name = CharField(max_length=64, null=True)    # 标题
    service_name = CharField(max_length=64, null=True)    # 对应的门店的服务名称：洗车服务，打蜡服务
    logo = CharField(max_length=256, null=True)  # logo
    desc = CharField(max_length=256, null=True)    # 描述
    setprice = IntegerField(default=0)  # 是否门店定义价格
    price = FloatField(default=0) # 平台价格
...
# 保养分类
class CarServiceProduct(db.Model):
    id = PrimaryKeyField()
    type = ForeignKeyField(CarServiceType, related_name='type_products', db_column='type_id')
    name = CharField(max_length=20)  # 分类名
    logo = CharField(max_length=256) # logo地址
    price = FloatField(default=0.0) # 价格
    active = IntegerField(default=1)  # 状态 0删除 1有效

    class Meta:
        db_table = 'tb_car_service_product'


class StoreService(db.Model):
    id = PrimaryKeyField()
    store = ForeignKeyField(Store, related_name='store_services', db_column='store_id')
    car_service_type = ForeignKeyField(CarServiceType, related_name='store_services', db_column='car_service_type_id')
...
# 汽车保养券发布
class CarServiceBox(db.Model):
    id = PrimaryKeyField()
    name = CharField(max_length=26)    # 套餐名称
    type = IntegerField(default=1)     # 类型:1单项 、2套餐
    total_num = IntegerField(default=0)
    out_num = IntegerField(default=0)
    buy_limit = IntegerField(default=1)   # 购买数量限制
    created = IntegerField(default=0)  # 创建时间
    price = FloatField(default=0)    # 销售价格：0免费领取 other响应价格购买
    overdue_time = IntegerField(default=0)  # 过期时间增量
    hot = IntegerField(default=0)    # 是否首页推广：0否1是
    activity_id = IntegerField(default=0)    # 参加某个活动
    sort = IntegerField(default=1)    # 优先级别：越大月靠前
    status = IntegerField(default=1)     # 状态：-1无效，1有效

    class Meta:
        db_table = 'tb_car_service_box'


# 汽车保养券发布
class CarServiceBoxItem(db.Model):
    id = PrimaryKeyField()
    type = ForeignKeyField(CarServiceType, related_name='type_items', db_column='type_id')  # 关联单项
    product = ForeignKeyField(CarServiceProduct, db_column='product_id',null=True)  # 关联单项
    box = ForeignKeyField(CarServiceBox, related_name='box_items', db_column='box_id')  # 所属套餐
    num = IntegerField(default=1)    # 单项对应数量

    class Meta:
        db_table = 'tb_car_service_box_item'


# 优惠券列表
class Coupons(db.Model):
    id = PrimaryKeyField()
    user = ForeignKeyField(User, db_column='user_id', related_name='coupons')  # 使用者
    activity = IntegerField(default=0)    # 活动ID，参加某个活动给的优惠券
    type = IntegerField(default=0)  # 0全部可用，1保养优惠券，2，3洗车优惠券 10 保险用优惠券
    car_service_box = ForeignKeyField(CarServiceBox, db_column='car_service_box_id', null=True)  # box
    store_service = ForeignKeyField(StoreService, db_column='store_service_id', null=True)  # 服务
    order = IntegerField(default=0)  # 购买优惠券订单
    pic = CharField(max_length=64)  # 优惠券图片
    price = FloatField(default=0)  # 优惠价格
    sort = IntegerField(default=0)  # 排序
    intro = CharField(max_length=256)  # 说明简介
    created = IntegerField(default=0)  # 创建时间
    used = IntegerField(default=0)  # 使用时间
    active = IntegerField(default=1)  # 用户状态 0未激活 1可用 2已使用
...
# 保养券订单
class CarServiceCardOrder(db.Model):
    id = PrimaryKeyField()
    user = ForeignKeyField(User, related_name='user_service_orders', db_column='user_id')  # 使用者
    ordernum = CharField(default='')
    trade_no = CharField(default='')
    total_price = FloatField(default=0.0)  # 订单总价格
    coupons = ForeignKeyField(Coupons, db_column='coupons_id', null=True)    # 优惠券
    created = IntegerField(default=0)
    pay_time = IntegerField(default=0)  # 支付时间
    status = IntegerField(default=0)  # 状态：0 未支付，1已支付

    class Meta:
        db_table = 'tb_car_service_card_order'
...
class CarServiceCard(db.Model):
    ...
    order = ForeignKeyField(CarServiceCardOrder, related_name='cards', db_column='order_id', null=True)  # 购买优惠券订单
    # 服务券信息采用复制保存副本，预防服务券类型变更引起用户持有服务券信息同步变更
    type = ForeignKeyField(CarServiceType, related_name='type_cards', db_column='type_id')  # 类型
    car_service_box_id = IntegerField(default=0)  # CarServiceBox id
    name = CharField(max_length=64, null=True)  # 标题
    desc = CharField(max_length=256, null=True)  # 描述
    created = IntegerField(default=0)  # 创建时间
    ...
    random_num = CharField(max_length=15, unique=True)  # 随机数不可重复
    come_from_flag = IntegerField(null=True)  # 来源flag 1绑定车辆时送的 2买保险时送的
    come_from_log = CharField(null=True)  # 来源log
    # 备用字段
    backup1 = CharField(null=True)  # SK产品名
    backup2 = CharField(null=True)  # SK产品介绍
    backup3 = CharField(null=True)  # api级别
    backup4 = CharField(null=True)  # SK产品logo
    backup5 = CharField(null=True)  # 数量
    ...
    def get_random_num():
        import random
        while 1:
            random_num = random.randrange(100000000000, 999999999999)
            if not CarServiceCard.select().where(CarServiceCard.random_num == random_num).count() > 0:
                return random_num
...
class Activity(db.Model):
    ...
    check_func = CharField()
    apply_to = CharField(max_length=16)    # 适用于 WX 微信，B b端修理厂
    intro = CharField(max_length=64)    # 图文详情的html路径
    policy = TextField(default='')  # 店铺返佣政策（汽车服务卡券）的json串：模仿返油。实现。
    active = IntegerField(default=1)    # 0失效 1生效 2过期
...
class StoreAddress(db.Model):
    ...
    created = IntegerField(default=0)  # 创建时间
    create_by = ForeignKeyField(User, related_name='user_addresses', db_column='user_id')  # 创建人
...
class Product(db.Model):
    ...
    buylimit = IntegerField(default=0) # 起订桶数限制：0不限制，1-无穷大限制的桶数
    unit = CharField()  # 单位
...
class InsuranceArea(db.Model):
    ...
    insurance = ForeignKeyField(Insurance, related_name='insurance_areas', db_column='insurance_id')
...
class InsuranceOrderPrice(db.Model):
    ...
    gift_policy = IntegerField(default=0)  # 礼品策略 1反油， 2反现金，3返积分，4优惠券
    ...
    # original_price = FloatField(default=0.0)  # 原价，使用优惠券之前的价格
    # coupon_id = IntegerField(default=0)  # 优惠券ID
    ...
    refund_status = IntegerField(default=0)    # 退款状态 0无退款 1已退款
    refund_time = CharField(default='')    # 退款时间
    should_refund_num = CharField(default='')    # 退款金额
    refund_num = CharField(default='')    # 实返佣金
    refund_lube = CharField(default='')    # 实退油品
    refund_time_real = CharField(default='')    # 实退时间
    ...
    @staticmethod
    def get_insurance_type(insurance_order_price):
        if insurance_order_price.force_price > 0 and insurance_order_price.business_price > 0:
            flag = 3
            i_type = u'交强+商业'
        elif insurance_order_price.force_price > 0:
            flag = 1
            i_type = u'单交强'
        elif insurance_order_price.force_price > 0:
            flag = 2
    ...
    @staticmethod
    def get_gift_policy(insurance_order_price):
        if insurance_order_price.gift_policy == 1:
            gift_policy = u'返油'
        elif insurance_order_price.gift_policy == 2:
            gift_policy = u'返现'
        elif insurance_order_price.gift_policy == 3:
            gift_policy = u'返积分'
        elif insurance_order_price.gift_policy == 3:
            gift_policy = u'返优惠券'
        else:
...
class InsuranceOrder(db.Model):
    id = PrimaryKeyField()
    ordernum = CharField(max_length=64, null=True)  # 订单号
    platform = CharField(max_length=64, null=True)  # 发起平台 W 微信，
    area_code = CharField(max_length=40)    # 保险出单区域
    sendservicecard = IntegerField(default=0) # 是否送保养券订单： 0否 1是
    ...
    coupon_id = IntegerField(default=0)  # 优惠券ID
    coupon_price = FloatField(default=0.0)  # 优惠券价格
...
# 个人车库
class UserCarLib(db.Model):
    id = PrimaryKeyField()
    car_item = ForeignKeyField(CarItem, related_name='car_item_users', db_column='car_item_id', null=True)  # 车型信息
    user = ForeignKeyField(User, related_name='user_car_libs', db_column='user_id')  # 车主
    # 其它补充信息
    ordered = IntegerField()    # 购买时间
    mileage = IntegerField(default=0)    # 行驶里程
    default = IntegerField(default=0)
    status = IntegerField(default=0)    # 0无效 1有效

    plate_num = CharField(default=0)    # 车牌号
    car_front = CharField(max_length=255, null=True)  # 身份证 投保人
    car_back = CharField(max_length=255, null=True)  # 身份证背面 投保人
    drive_card_front = CharField(max_length=255, null=True)  # 行驶证
    drive_card_back = CharField(max_length=255, null=True)  # 行驶证反面
    # OCR 结果
    config_str = CharField(null=True)
    engine_num = CharField(null=True)
    issue_date = CharField(null=True)
    model = CharField(null=True)
    owner = CharField(null=True)
    plate_num_ocr = CharField(null=True)
    register_date = CharField(null=True)
    request_id = CharField(null=True)
    use_character = CharField(null=True)
    vehicle_type = CharField(null=True)
    vin = CharField(null=True)

    class Meta:
        db_table = 'tb_user_car_lib'
